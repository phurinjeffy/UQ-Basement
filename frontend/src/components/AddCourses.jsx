...
function AddCourses({ onConfirm, initialCourses = [] }) {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-8 bg-white/90 backdrop-blur-lg rounded-3xl shadow-xl border border-white/20">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent mb-2">
          Add Your Courses
        </h2>
        <p className="text-gray-600">Search and select the courses you're taking this semester</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <div className="relative">
            <input
              type="text"
              className="w-full px-4 py-3 bg-white/50 backdrop-blur-sm rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-all duration-200 outline-none"
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-gray-200 shadow-lg overflow-hidden">
              {suggestions.map((s, idx) => (
                <button
                  key={idx}
                  type="button"
                  className="w-full p-4 text-left hover:bg-indigo-50 transition-colors duration-150 border-b border-gray-100 last:border-b-0"
                  onClick={() => addCourse(s)}
                >
                  <div className="font-semibold text-gray-900">{s.name}</div>
                  <div className="text-sm text-gray-600 mt-1">{s.course_title}</div>
                </button>
              ))}
            </div>
          )}
        ...
        <div>
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Selected Courses ({courses.length})</h3>
          <div className="max-h-96 overflow-y-auto space-y-3 scrollbar-hide">
            {courses.length === 0 ? (
              ...
              courses.map((course, idx) => (
                <div key={idx} className="bg-white/60 backdrop-blur-sm rounded-xl p-4 border border-gray-200 relative hover-lift">
                  <button
                    type="button"
                    className="absolute top-3 right-3 w-6 h-6 rounded-full bg-red-100 text-red-600 hover:bg-red-600 hover:text-white transition-all duration-200 flex items-center justify-center text-sm font-medium"
                  ...
                  <div className="pr-8 mb-3">
                    ...
                    <div className="text-sm text-gray-600 line-clamp-2">{course.course_title}</div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        type="date"
                        className="w-full px-3 py-2 text-sm bg-white/70 rounded-lg border border-gray-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200 transition-all duration-200 outline-none"
